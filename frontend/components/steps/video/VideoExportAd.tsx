...
const generateAd = async () => {
    try {
      ...
      // 验证必要参数
      if (!formData.video || selectedPlatforms.length === 0) {
        setErrorMessage("Please select video and platform(s) before generating.");
        setGenerating(false);
        return;
      }
      ...
      // 验证project_id是否存在
      if (!formData.project_id) {
        setErrorMessage("Project ID is missing. Please try uploading the video again.");
        setGenerating(false);
        return;
      }
